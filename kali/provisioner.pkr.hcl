locals {
  ...
  boot_command = length(var.boot_command) == 0 ? [
    ...
    "auto=true url=http://{{.HTTPIP}}:{{.HTTPPort}}/kali/preseed.cfg priority=critical<f10><wait>"
  ] : var.boot_command
...
source "parallels-iso" "image" {
  ...
  http_content = {
    "/kali/meta-data"          = templatefile("${path.root}/../http/kali/meta-data.pkrtpl.hcl", { hostname = "${local.hostname}" })
    "/kali/preseed.cfg"        = templatefile("${path.root}/../http/kali/preseed.cfg.pkrtpl.hcl", { username = "${local.username}", password = "${local.password}", hostname = "${local.hostname}", desktop = "${var.desktop}" })
  }
