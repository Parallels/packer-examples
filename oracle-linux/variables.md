...
This guide provides detailed explanations of the variables that can be set in the `variables.pkrvars.hcl` file for the Oracle Linux Virtual Machine Builder.
...
* `machine_name` - Defines the name of the virtual machine in Parallels Desktop. If empty, defaults to `ol Server <version>`.
* `hostname` - Sets the hostname for the virtual machine. If empty, defaults to `ol-<version>` format.
...
* `boot_command` - Determines the boot command used when starting the virtual machine. A default value is provided for Oracle Linux. You can specify your own if needed.
* `boot_wait` - Determines the duration the script should wait for the virtual machine to boot. Default is `10s`.

## Oracle Linux OS Source

* `iso_url` - Specifies the path to the Oracle Linux ISO file or the URL for it to be downloaded. This is a required variable.
...
* `version` - Specifies the version of Oracle Linux being installed. Default is `2023.3`.
