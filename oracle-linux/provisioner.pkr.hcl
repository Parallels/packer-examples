locals {
  ...
  hostname = var.hostname == "" ? "ol-${local.version}" : var.hostname

  boot_command = length(var.boot_command) == 0 ? [
    "<wait>",
    "<up>e<wait>",
    // "<bs><bs><bs><bs><bs><bs><bs><bs><bs><bs><bs><bs><bs>",
    "<down><down><end> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ol/ks.cfg<F10><wait>"
  ] : var.boot_command
  ...
  machine_name = var.machine_name == "" ? "ol Server ${local.version}" : var.machine_name
...
source "parallels-iso" "image" {
  ...
  http_content = {
    "/ol/ks.cfg" = templatefile("${path.root}/../http/oracle-linux/ks.cfg.pkrtpl.hcl", { username = "${local.username}", password = "${local.password}", hostname = "${local.hostname}", package = "${var.install_desktop ? "@^graphical-server-environment" : "@^server-product-environment"}" })
  }
