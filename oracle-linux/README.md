# Automated Oracle Linux Virtual Machine Builder with Packer and Vagrant for Apple Silicon Macs & Parallels Desktop

Create virtual Oracle Linux machines or Vagrant boxes in an automated, customizable fashion using Packer. This tool is designed specifically for Macs with Apple Silicon chip and leverages Parallels Desktop.

## Overview

This project provides automated Packer scripts to build Oracle Linux virtual machines or Vagrant boxes. It offers the flexibility to use any of the available ISO's for the distro. The selected source can be defined by setting the `iso_url` variable in the `variables.pkrvars.hcl` file. By default, the script will use and auto-download the ISO for Oracle Linux 2023.3 if no source is specified in the `variables.pkrvars.hcl` file.

Oracle Linux is available freely from [here](https://www.oracle.com/linux/).
...
```hcl
version="2023.3"
machine_name="Oracle Linux Server"
hostname="ol-2023-3"
machine_specs = {
  cpus = 2,
  memory = 2048,
