build {
  ...
  provisioner "shell" {
    ...
    scripts = [
      "${path.root}/../scripts/centos/base/sshd.sh",
      "${path.root}/../scripts/centos/base/update.sh",
      "${path.root}/../scripts/centos/base/parallels.sh",
      "${path.root}/../scripts/centos/base/parallels_folders.sh",
    ]
  ...
  provisioner "shell" {
    scripts = [
      "${path.root}/../scripts/centos/base/desktop.sh",
    ]
  ...
  provisioner "shell" {
    environment_vars = [
      "ADDONS=${local.addons}",
      "ADDONS_DIR=${path.root}/../scripts/centos/addons"
    ]
    scripts = [
      "${path.root}/../scripts/centos/addons/install.sh",
    ]
    execute_command   = "echo '${local.password}' | {{ .Vars }} sudo -S -E bash -eux '{{ .Path }}'"
    expect_disconnect = true
    timeout           = "3h"
  ...
  provisioner "shell" {
    ...
    scripts = [
      "${path.root}/../scripts/centos/base/password_change.sh",
      "${path.root}/../scripts/centos/base/cleanup.sh",
    ]
    execute_command   = "echo '${local.password}' | {{ .Vars }} sudo -S -E sh -eux '{{ .Path }}'"
    expect_disconnect = true
