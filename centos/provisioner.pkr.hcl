locals {
  ...
  hostname = var.hostname == "" ? "centos-${local.version}" : var.hostname

  boot_command = [
    "<wait5>", # Added wait to ensure network is up
    "<up>e<wait>",
    "<down><down><end> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos/ks.cfg<F10><wait>"
  ]
  ...
  machine_name = var.machine_name == "" ? "centos Server ${local.version}" : var.machine_name
...
source "parallels-iso" "image" {
  ...
  http_content = {
    "/centos/ks.cfg" = templatefile("${path.root}/../http/centos/ks.cfg.pkrtpl.hcl", { username = "${local.username}", password = "${local.password}", hostname = "${local.hostname}", package = "${var.install_desktop ? "@^graphical-server-environment" : "@^server-product-environment"}" })
  }
