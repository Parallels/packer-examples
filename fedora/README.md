# Automated fedora
 Virtual Machine Builder with Packer and Vagrant for Apple Silicon Macs & Parallels Desktop
...
This project provides automated Packer scripts to build fedora virtual machines or Vagrant boxes. It offers the flexibility to use any of the available ISO's for the distro. The selected source can be defined by setting the `iso_url` variable in the `variables.pkrvars.hcl` file. By default, the script will use and auto-download the ISO for fedora
 2023.3 if no source is specified in the `variables.pkrvars.hcl` file.
Because fedora
 is a commercial distro, you will need to provide your own iso, you can source a trial version from [here](https://www fedora
.com/en/technologies/linux-platforms/enterprise-linux).
You also will need to provide a fedora username/password if you want to install any addons has this is required by fedora
